...
async function refreshMesas() {
    ...
    data.forEach(m => {
        ...
        div.onclick = () => abrirModalAcciones(m.mesa_id, m.num_mesa, m.cuenta_id, isBusy);
...
async function accionCerrarCuenta() {
    ...
    if(!confirm("¿Confirmar cobro y cierre de cuenta?")) return;
    try {
        const res = await fetch(`${API_URL}/cuentas/cerrar/${cuentaSeleccionadaId}`, { method: 'PUT' });
        if(res.ok) {
            cerrarModal();
            refreshMesas();
...
async function accionCancelarCuenta() {
    if(!cuentaSeleccionadaId) return;
    if(!confirm("¿SEGURO? Esto eliminará la cuenta y sus pedidos de la base de datos.")) return;
    try {
        const res = await fetch(`${API_URL}/cuentas/cancelar/${cuentaSeleccionadaId}`, { method: 'DELETE' });
        if(res.ok) {
            cerrarModal();
            refreshMesas();
